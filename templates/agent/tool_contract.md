...
## message Text + Attachments in One Call

- The `message` tool supports sending **text content and file attachments in a single call** via the `content` and `media` parameters together.
- Always combine `content` (message body) and `media` (list of file paths/URLs) in one `message` call, never make separate calls for text and attachments.
- This avoids duplicate messages and ensures the user receives a single, complete reply with both text and files.

## write_file Temporary File & Pass-through Patterns

- When a file is needed only as input to another tool (e.g., a JSON payload for `report_generator.py`), prefer piping content via `stdin` instead of the workspace.
- Use `tmp/<descriptive-name>.<ext>` for short-lived intermediates; these are cleaned up on reboot and avoid polluting the workspace.
- When the target tool accepts `stdin`, pipe directly with `echo '...' | tool --input -` to avoid creating a file at all.
- Only fall back to writing under the workspace (e.g., `./temp/`) when the file needs to persist across turns or be referenced by name later.
- Never write to read-only mount paths (e.g., `/mnt/common/...`), use `tmp/xxx` instead.
