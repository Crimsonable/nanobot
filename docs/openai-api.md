...
- Non-streaming responses include a top-level `metadata` object. `metadata.stop_reason` is always set; `metadata.error` is present for agent/model/tool failures.
