...
async def test_message_tool_uploads_local_bridge_media_via_container_up(
    ...
    monkeypatch.setattr(
        "nanobot.utils.document._materialize_attachment_payload",
        lambda ref, metadata=None: {
            "url": f"https://files.example/{Path(str(ref)).name}",
            "filename": Path(str(ref)).name,
...
@pytest.mark.asyncio
async def test_message_tool_normalizes_remote_bridge_media_to_attachment_objects(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    sent: list[OutboundMessage] = []
...
async def test_message_tool_reports_bridge_upload_failure(
    ...
    assert result.startswith("Error sending message: failed to normalize attachment via container_up:")
    assert sent == []
